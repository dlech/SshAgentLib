...
namespace dlech.SshAgentLib
{
  ...
  public abstract class Agent : IAgent, IDisposable
  {
    ...
    public void AnswerMessage(Stream aMessageStream)
    {
      ...
      switch (header.Message) {
        ...
        case Message.SSH1_AGENTC_RSA_CHALLENGE:
           ...
           */
            ...
            try
            {
                ...
                ISshKey matchingKey = mKeyList.Where(key => key.Version == SshVersion.SSH1 && (key.GetPublicKeyParameters().Equals(publicKeyParams))).Single();

                //Reading challenge
                BlobParser ssh1ChallengeParser = new BlobParser(aMessageStream);
                PinnedByteArray encryptedChallenge = ssh1ChallengeParser.ReadSsh1Blob();
                PinnedByteArray sessionId = ssh1ChallengeParser.ReadBlob(16);

                //Checking responseType field
                if (ssh1ChallengeParser.ReadInt() != 1)
                {
                    goto default; //Protocol 1.0 is not supported anymore
                }
                ...
                byte[] decryptedChallenge = engine.ProcessBlock(encryptedChallenge.Data, 0, encryptedChallenge.Data.Length);
        ...
        case Message.SSH1_AGENTC_ADD_RSA_IDENTITY:
          ...
          bool ssh1constrained = (header.Message == Message.SSH1_AGENTC_ADD_RSA_IDENTITY);
    ...
    public static AsymmetricKeyParameter ParseSsh1PublicKeyData(Stream aSteam, bool aReverseRsaParameters)
    {
        ...
        var rsaN = new BigInteger(1, parser.ReadSsh1Blob().Data);
        var rsaE = new BigInteger(1, parser.ReadSsh1Blob().Data);
    ...
    public static AsymmetricCipherKeyPair ParseSsh1KeyData(AsymmetricKeyParameter aPublicKeyParameter, Stream aSteam)
    {
        ...
        PinnedByteArray rsa_d = parser.ReadSsh1Blob();
        PinnedByteArray rsa_iqmp = parser.ReadSsh1Blob();
        PinnedByteArray rsa_q = parser.ReadSsh1Blob();
        PinnedByteArray rsa_p = parser.ReadSsh1Blob();
